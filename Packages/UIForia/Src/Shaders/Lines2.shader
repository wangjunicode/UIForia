        ...
        Pass
        {
           ...
           struct v2f {
            ...
            v2f vert (appdata v) {
                ...
                // to clip miter or do round join
                    // find miter top point
                    // dirToTop = realMiterPoint - curr; // get direction from current to top
                    // newTopPoint = curr + dirToTop * min(miterLimit, miterLength);
                    // point4 = LineIntersection(point1, miterTop, newTopPoint, newTopPoint + normalize(perp(newTopPoint))
                    // point7 = LineIntersection(point0, miterTop, newTopPoint, newTopPoint + normalize(perp(newTopPoint))
