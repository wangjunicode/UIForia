...
namespace UIForia.Style {
    ...
    internal struct TextureInfoWrite {
    ...
    internal unsafe struct PropertySolverInfo_TextureInfo {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_TextureInfo* activeTransitionList;
        ...
        public DataList<PropertySolver_TextureInfo.AnimationValue_TextureInfo> animationValues;

    }

    internal static unsafe class PropertySolver_TextureInfo {

        public static void Invoke(ref PropertySolverInfo_TextureInfo solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<TextureInfo> styleDatabase, TextureInfo* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_TextureInfo value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_TextureInfo> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<TextureInfoWrite> writeBuffer = tempAllocator->AllocateList<TextureInfoWrite>(maxWriteBufferSize);
            CheckedArray<TextureVariable> variables = SolverParameters.GetVariableList_TextureVariable(parameters);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_TextureInfo transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<TextureInfo> transitionPrevValues = tempAllocator->AllocateList<TextureInfo>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        TextureInfo next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        TextureInfo prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromTextureInfo(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        TextureInfo result = TextureInfo_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_TextureInfo active = new ActiveTransition_TextureInfo {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_TextureInfo prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_TextureInfo.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromTextureInfo(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_TextureInfo {

            public ElementId elementId;
            public TextureInfo currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_TextureInfo solverInfo, in SolverParameters parameters, TextureInfo defaultValue, TextureInfo* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                TextureInfo* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, TextureInfo* outputTargets, BumpList<TextureInfoWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref TextureInfoWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_TextureInfo solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_TextureInfo transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_TextureInfo> SetupFrameData(ref PropertySolverInfo_TextureInfo solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_TextureInfo> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_TextureInfo>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, TextureInfo* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<TextureInfoWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                TextureInfo value = update.Get<TextureInfo>();
                ...
                writeBuffer.array[writeBuffer.size++] = new TextureInfoWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_TextureInfo solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<TextureInfoWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_TextureInfo.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new TextureInfoWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_TextureInfo.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static TextureInfo FindInWriteBuffer(ElementId elementId, BumpList<TextureInfoWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_TextureInfo FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_TextureInfo> list) {
        ...
        private static TextureInfo ResolveAnimationVariable(ushort variableNameId, TextureInfo defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<TextureVariable> variables, ElementId elementId) {
            ...
            for (int i = 0; i < variables.size; i++) {
                TextureVariable variable = variables[i];
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToTextureInfo(out TextureInfo resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<TextureVariable> variables, ref BumpList<TextureInfoWrite> writeBuffer, ElementMap changeMap, TextureInfo* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<TextureInfo> styleDatabase,
            TextureInfo* outputTargets,
            ...
            ref BumpList<TextureInfoWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                TextureInfo variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            TextureInfo* values = tempAllocator->Allocate<TextureInfo>(usedUpdateSize);
            TextureInfo* dbValues = tempAllocator->Allocate<TextureInfo>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                TextureInfo databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                TextureInfo* current = outputTargets + update.elementId.index;
