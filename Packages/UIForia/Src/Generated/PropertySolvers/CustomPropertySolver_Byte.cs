...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_Byte : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<Byte>> transitions;
        public UnsafeHashMap<ElementId, Byte> valueMap;

        public Byte defaultValue;

        public Byte GetValue(ElementId elementId, Byte defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out Byte retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<Byte> dbValues = propertyTables.propertyTable_Byte;
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<Byte> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out Byte prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<Byte>() {
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                Byte dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<ValueVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<Byte> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out Byte currentValue)) {
                ...
                Byte next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                Byte prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<Byte> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out Byte currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        Byte output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = Byte_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private Byte ResolveAnimationVariable(ushort nextVarId, Byte current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<ValueVariable> variableList, ElementId elementId) {
            ...
            for (int v = 0; v < variableList.size; v++) {
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertToByte(out resolved)) {
                    continue;
                }
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, Byte>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<Byte>>(8, Allocator.Persistent);
        }
