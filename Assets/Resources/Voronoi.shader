   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         fixed4 frag(v2f i) : SV_Target
         {
            ...
            minDist = smoothstep(-1, 1, minDist);
            // Draw the min distance (distance field)
            col.r += minDist * minDist; // squared it to to make edges look sharper
            col.g += minDist * minDist; // squared it to to make edges look sharper
            col.b += minDist * minDist; // squared it to to make edges look sharper
            
            return col * (_Tint); // * fixed4(0.5, 0.5, 0.5, 0.5);
         }
      ENDCG
      }
   }
