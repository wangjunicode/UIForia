...
namespace UIForia.Editor.FontLoading {
    ...
    public class FontCreator {
        ...
        public class FontCreatorCreatorWindow : EditorWindow {
            ...
            private UIForiaGlyph[] GetGlyphInfo(FontLoader.FT_GlyphInfo[] ftGlyphs) {
                List<UIForiaGlyph> glyphs = new List<UIForiaGlyph>(ftGlyphs.Length);
            ...
            public TextKerningPair[] GetKerningTable(string fontFilePath, int pointSize, UIForiaGlyph[] glyphs) {

                FontLoader.FT_KerningPair[] kerningPairs = new FontLoader.FT_KerningPair[7500];

                int[] kerningSet = new int[glyphs.Length];
                for (int i = 0; i < glyphs.Length; i++) {
                    kerningSet[i] = glyphs[i].codepoint;
                }
                
                int kpCount = FontLoader.TMPro_FontPlugin.FT_GetKerningPairs(fontFilePath, kerningSet, kerningSet.Length, kerningPairs);

                TextKerningPair[] retn = new TextKerningPair[kpCount];
                for (int i = 0; i < kpCount; i++) {
                    retn[i] = new TextKerningPair() {
                        firstGlyph = kerningPairs[i].ascII_Left,
                        secondGlyph = kerningPairs[i].ascII_Right,
