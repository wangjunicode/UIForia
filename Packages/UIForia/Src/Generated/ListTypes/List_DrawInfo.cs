...
namespace UIForia.ListTypes {
    ...
    [DebuggerTypeProxy(typeof(DebugView_DrawInfo))]
    internal unsafe struct List_DrawInfo : IBasicList<DrawInfo> {

        public int size;
        private ushort capacityShiftBits;
        private AllocatorUShort allocator;

        [NativeDisableUnsafePtrRestriction] public DrawInfo* array;

        private const int k_MinCapacity = 4;

        public List_DrawInfo(int initialCapacity, Allocator allocator, bool clearMemory = false) {
        ...
        private void Initialize(int capacity, Allocator allocator, bool clearMemory) {
            ...
            this.array = (DrawInfo*) UnsafeUtility.Malloc(sizeof(DrawInfo) * capacity, UnsafeUtility.AlignOf<DrawInfo>(), allocator);
            this.capacityShiftBits = (ushort) BitUtil.GetPowerOfTwoBitIndex((uint) capacity);
            if (clearMemory) {
                UnsafeUtility.MemClear(array, sizeof(DrawInfo) * capacity);
            }
        }

        public ref DrawInfo this[int index] {
            get => ref array[index];
        }
        ...
        public void Add(in DrawInfo item) {
        ...
        public void AddUnchecked(in DrawInfo item) {
            array[size++] = item;
        }

        public void AddRange(DrawInfo* items, int itemCount) {
        ...
        public void EnsureCapacity(int desiredCapacity, bool clearMemory = false) {
            ...
            long bytesToMalloc = sizeof(DrawInfo) * capacity;
            void* newPointer = UnsafeUtility.Malloc(bytesToMalloc, UnsafeUtility.AlignOf<DrawInfo>(), fullAllocator);
            ...
            if (array != default) {
                int bytesToCopy = size * sizeof(DrawInfo);
        ...
        public ref DrawInfo Get(int index) {
            return ref array[index];
        }

        public void Set(in DrawInfo item, int index) {
            array[index] = item;
        }
        ...
        public DrawInfo* GetPointer(int index) {
            return array + index;
        }
        ...
        public DrawInfo GetLast() {
            return array[size - 1];
        }

        public void CopyFrom(DrawInfo* data, int count) {
            EnsureCapacity(count);
            size = count;
            TypedUnsafe.MemCpy(array, data, size);
        }

        public void CopyFrom(DrawInfo* data, int count, Allocator allocator) {
            if (array == null) {
        ...
        public struct Shared : IDisposable {
            ...
            [NativeDisableUnsafePtrRestriction] public List_DrawInfo* state;

            public Shared(int initialCapacity, Allocator allocator, bool clear = false) {
                this.state = TypedUnsafe.Malloc<List_DrawInfo>(1, allocator);
                *this.state = new List_DrawInfo(initialCapacity, allocator, clear);
            }
            ...
            public ref DrawInfo GetReference(int index) {
            ...
            public DrawInfo* GetArrayPointer() {
                return state->array;
            }
            ...
            public void FilterSwapRemove<TFilter>(TFilter filter) where TFilter : IListFilter<DrawInfo> {
        ...
        public struct DebugView_DrawInfo {

            public int size;
            public int capacity;
            public DrawInfo[] data;

            public DebugView_DrawInfo(List_DrawInfo target) {
                this.size = target.size;
                this.capacity = target.Capacity;
                this.data = new DrawInfo[size];
                for (int i = 0; i < size; i++) {
                    data[i] = target[i];
                }
            }

            public DebugView_DrawInfo(List_DrawInfo.Shared target) {
                this.size = target.size;
                this.capacity = target.capacity;
                this.data = new DrawInfo[size];
                for (int i = 0; i < size; i++) {
