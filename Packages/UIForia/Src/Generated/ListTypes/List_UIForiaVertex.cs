...
namespace UIForia.ListTypes {
    ...
    [DebuggerTypeProxy(typeof(DebugView_UIForiaVertex))]
    internal unsafe struct List_UIForiaVertex : IBasicList<UIForiaVertex> {

        public int size;
        private ushort capacityShiftBits;
        private AllocatorUShort allocator;

        [NativeDisableUnsafePtrRestriction] public UIForiaVertex* array;

        private const int k_MinCapacity = 4;

        public List_UIForiaVertex(int initialCapacity, Allocator allocator, bool clearMemory = false) {
        ...
        private void Initialize(int capacity, Allocator allocator, bool clearMemory) {
            ...
            this.array = (UIForiaVertex*) UnsafeUtility.Malloc(sizeof(UIForiaVertex) * capacity, UnsafeUtility.AlignOf<UIForiaVertex>(), allocator);
            this.capacityShiftBits = (ushort) BitUtil.GetPowerOfTwoBitIndex((uint) capacity);
            if (clearMemory) {
                UnsafeUtility.MemClear(array, sizeof(UIForiaVertex) * capacity);
            }
        }

        public ref UIForiaVertex this[int index] {
            get => ref array[index];
        }
        ...
        public void Add(in UIForiaVertex item) {
        ...
        public void AddUnchecked(in UIForiaVertex item) {
            array[size++] = item;
        }

        public void AddRange(UIForiaVertex* items, int itemCount) {
        ...
        public void EnsureCapacity(int desiredCapacity, bool clearMemory = false) {
            ...
            long bytesToMalloc = sizeof(UIForiaVertex) * capacity;
            void* newPointer = UnsafeUtility.Malloc(bytesToMalloc, UnsafeUtility.AlignOf<UIForiaVertex>(), fullAllocator);
            ...
            if (array != default) {
                int bytesToCopy = size * sizeof(UIForiaVertex);
            ...
            array = (UIForiaVertex*) newPointer;
        }
        ...
        public ref UIForiaVertex Get(int index) {
            return ref array[index];
        }

        public void Set(in UIForiaVertex item, int index) {
            array[index] = item;
        }
        ...
        public UIForiaVertex* GetPointer(int index) {
            return array + index;
        }
        ...
        public UIForiaVertex GetLast() {
            return array[size - 1];
        }

        public void CopyFrom(UIForiaVertex* data, int count) {
            EnsureCapacity(count);
            size = count;
            TypedUnsafe.MemCpy(array, data, size);
        }

        public void CopyFrom(UIForiaVertex* data, int count, Allocator allocator) {
            if (array == null) {
        ...
        public struct Shared : IDisposable {
            ...
            [NativeDisableUnsafePtrRestriction] public List_UIForiaVertex* state;

            public Shared(int initialCapacity, Allocator allocator, bool clear = false) {
                this.state = TypedUnsafe.Malloc<List_UIForiaVertex>(1, allocator);
                *this.state = new List_UIForiaVertex(initialCapacity, allocator, clear);
            }
            ...
            public ref UIForiaVertex GetReference(int index) {
            ...
            public UIForiaVertex* GetArrayPointer() {
                return state->array;
            }
            ...
            public void FilterSwapRemove<TFilter>(TFilter filter) where TFilter : IListFilter<UIForiaVertex> {
        ...
        public struct DebugView_UIForiaVertex {

            public int size;
            public int capacity;
            public UIForiaVertex[] data;

            public DebugView_UIForiaVertex(List_UIForiaVertex target) {
                this.size = target.size;
                this.capacity = target.Capacity;
                this.data = new UIForiaVertex[size];
                for (int i = 0; i < size; i++) {
                    data[i] = target[i];
                }
            }

            public DebugView_UIForiaVertex(List_UIForiaVertex.Shared target) {
                this.size = target.size;
                this.capacity = target.capacity;
                this.data = new UIForiaVertex[size];
                for (int i = 0; i < size; i++) {
