...
namespace UIForia.Util {
    ...
    public class ClassBuilder {
        ...
        public ClassBuilder(TypeResolver typeResolver = null) {
            this.typeResolver = typeResolver ?? TypeResolver.Default;
        ...
        public Type CreateRuntimeType(string id, Type baseType, IList<ReflectionUtil.FieldDefinition> fields, IList<ReflectionUtil.MethodDefinition> methods, IList<string> namespaces) {
            ...
            if (fields != null) {
                for (int i = 0; i < fields.Count; i++) {
                    Type fieldType = typeResolver.ResolveType(fields[i].fieldType, (IReadOnlyList<string>) namespaces);
            ...
            if (methods != null) {
                for (int i = 0; i < methods.Count; i++) {
                    ...
                    if (methodDefinition.returnType.typeName != null && methodDefinition.returnType.typeName != "void") {
                        returnType = typeResolver.ResolveType(methodDefinition.returnType, (IReadOnlyList<string>) namespaces);
                    }
                    ...
                    for (int j = 0; j < methodDefinition.arguments.Length; j++) {
                        ...
                        Type type = typeResolver.ResolveType(typeLookup.Value, (IReadOnlyList<string>) namespaces);

                        if (type == null) {
                            throw new Exception($"Unable to resolve type for {typeLookup.Value.ToString()}");
                        }
                    ...
                    if (methodDefinition.isStatic) {
                        ...
                        throw new Exception("Static methods are not yet supported in Dynamic element types");
                    }
