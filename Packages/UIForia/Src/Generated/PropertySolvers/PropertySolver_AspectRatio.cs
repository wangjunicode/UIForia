...
namespace UIForia.Style {
    ...
    internal struct AspectRatioWrite {
    ...
    internal unsafe struct PropertySolverInfo_AspectRatio {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_AspectRatio* activeTransitionList;
        ...
        public DataList<PropertySolver_AspectRatio.AnimationValue_AspectRatio> animationValues;

    }

    internal static unsafe class PropertySolver_AspectRatio {

        public static void Invoke(ref PropertySolverInfo_AspectRatio solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<AspectRatio> styleDatabase, AspectRatio* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_AspectRatio value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_AspectRatio> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<AspectRatioWrite> writeBuffer = tempAllocator->AllocateList<AspectRatioWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_AspectRatio transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<AspectRatio> transitionPrevValues = tempAllocator->AllocateList<AspectRatio>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        AspectRatio next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        AspectRatio prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromAspectRatio(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        AspectRatio result = AspectRatio_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_AspectRatio active = new ActiveTransition_AspectRatio {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_AspectRatio prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_AspectRatio.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromAspectRatio(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_AspectRatio {

            public ElementId elementId;
            public AspectRatio currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_AspectRatio solverInfo, in SolverParameters parameters, AspectRatio defaultValue, AspectRatio* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                AspectRatio* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, AspectRatio* outputTargets, BumpList<AspectRatioWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref AspectRatioWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_AspectRatio solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_AspectRatio transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_AspectRatio> SetupFrameData(ref PropertySolverInfo_AspectRatio solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_AspectRatio> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_AspectRatio>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, AspectRatio* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<AspectRatioWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                AspectRatio value = update.Get<AspectRatio>();
                ...
                writeBuffer.array[writeBuffer.size++] = new AspectRatioWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_AspectRatio solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<AspectRatioWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_AspectRatio.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new AspectRatioWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_AspectRatio.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static AspectRatio FindInWriteBuffer(ElementId elementId, BumpList<AspectRatioWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_AspectRatio FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_AspectRatio> list) {
        ...
        private static AspectRatio ResolveAnimationVariable(ushort variableNameId, AspectRatio defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToAspectRatio(out AspectRatio resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<AspectRatioWrite> writeBuffer, ElementMap changeMap, AspectRatio* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<AspectRatio> styleDatabase,
            AspectRatio* outputTargets,
            ...
            ref BumpList<AspectRatioWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                AspectRatio variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            AspectRatio* values = tempAllocator->Allocate<AspectRatio>(usedUpdateSize);
            AspectRatio* dbValues = tempAllocator->Allocate<AspectRatio>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                AspectRatio databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                AspectRatio* current = outputTargets + update.elementId.index;
