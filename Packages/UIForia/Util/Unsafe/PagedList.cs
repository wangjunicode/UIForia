...
namespace UIForia.Util.Unsafe {
    ...
    public unsafe struct PagedList<T> : IDisposable where T : unmanaged {
        ...
        public struct PerThread : IDisposable {
            ...
            public PagedList<T> GetListForThread(int threadIndex) {
                return new PagedList<T>(PerThreadPagedListUtil.GetListForThread(perThreadData, threadIndex, allocator, pageSize));
            }
        ...
        public struct PerThreadBatchData {
            ...
            public PagedList<T> GetPerBatchData(int batchIndex, int threadId) {
                for (int i = 0; i < totalBatches; i++) {

                    if (data[i].threadId == threadId) {
                        return new PagedList<T>((PagedListState*) data + i);
                    }
                ...
                return new PagedList<T>((PagedListState*) data[batchIndex].data);
            }
