...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_half : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<half>> transitions;
        public UnsafeHashMap<ElementId, half> valueMap;

        public half defaultValue;

        public half GetValue(ElementId elementId, half defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out half retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<half> dbValues = propertyTables.propertyTable_half;
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<half> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out half prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<half>() {
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                half dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<ValueVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<half> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out half currentValue)) {
                ...
                half next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                half prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<half> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out half currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        half output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = half_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private half ResolveAnimationVariable(ushort nextVarId, half current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<ValueVariable> variableList, ElementId elementId) {
            ...
            for (int v = 0; v < variableList.size; v++) {
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertTohalf(out resolved)) {
                    continue;
                }
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, half>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<half>>(8, Allocator.Persistent);
        }
