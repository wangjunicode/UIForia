...
namespace Layout {
    ...
    public class StackLayoutTests {
        ...
        public void ComputeContentSize() {
            using (MockApplication mockView = MockApplication.Setup<Stack_ComputeContentSize>()) {
                Stack_ComputeContentSize root = (Stack_ComputeContentSize) mockView.RootElement;

                mockView.Update();

                Assert.AreEqual(new Rect(0, 0, 150, 150), root.layoutResult.LocalRect);
        ...
        public void AlignItems() {
            using (MockApplication mockView = MockApplication.Setup<Stack_AlignItems>()) {
