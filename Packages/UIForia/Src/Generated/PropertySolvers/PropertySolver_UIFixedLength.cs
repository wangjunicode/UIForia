...
namespace UIForia.Style {
    ...
    internal struct UIFixedLengthWrite {
    ...
    internal unsafe struct PropertySolverInfo_UIFixedLength {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UIFixedLength* activeTransitionList;
        ...
        public DataList<PropertySolver_UIFixedLength.AnimationValue_UIFixedLength> animationValues;

    }

    internal static unsafe class PropertySolver_UIFixedLength {

        public static void Invoke(ref PropertySolverInfo_UIFixedLength solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UIFixedLength> styleDatabase, UIFixedLength* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UIFixedLength value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UIFixedLength> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UIFixedLengthWrite> writeBuffer = tempAllocator->AllocateList<UIFixedLengthWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIFixedLength transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UIFixedLength> transitionPrevValues = tempAllocator->AllocateList<UIFixedLength>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIFixedLength next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UIFixedLength prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUIFixedLength(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIFixedLength result = UIFixedLength_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UIFixedLength active = new ActiveTransition_UIFixedLength {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UIFixedLength prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UIFixedLength.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUIFixedLength(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UIFixedLength {

            public ElementId elementId;
            public UIFixedLength currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UIFixedLength solverInfo, in SolverParameters parameters, UIFixedLength defaultValue, UIFixedLength* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UIFixedLength* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UIFixedLength* outputTargets, BumpList<UIFixedLengthWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UIFixedLengthWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UIFixedLength solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIFixedLength transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UIFixedLength> SetupFrameData(ref PropertySolverInfo_UIFixedLength solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UIFixedLength> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UIFixedLength>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UIFixedLength* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UIFixedLengthWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UIFixedLength value = update.Get<UIFixedLength>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UIFixedLengthWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UIFixedLength solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UIFixedLengthWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UIFixedLength.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UIFixedLengthWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UIFixedLength.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UIFixedLength FindInWriteBuffer(ElementId elementId, BumpList<UIFixedLengthWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UIFixedLength FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UIFixedLength> list) {
        ...
        private static UIFixedLength ResolveAnimationVariable(ushort variableNameId, UIFixedLength defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUIFixedLength(out UIFixedLength resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UIFixedLengthWrite> writeBuffer, ElementMap changeMap, UIFixedLength* outputTargets) {
            ...
            for (int updateIdx = 0; updateIdx < propertyList.size; updateIdx++) {
                ...
                if (start == -1) {
                    ...
                    writeBuffer.array[writeBuffer.size++] = new UIFixedLengthWrite() {
                        elementId = update.elementId,
                        value = update.Get<UIFixedLength>(),
                    };
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UIFixedLength> styleDatabase,
            UIFixedLength* outputTargets,
            ...
            ref BumpList<UIFixedLengthWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UIFixedLength variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UIFixedLength* values = tempAllocator->Allocate<UIFixedLength>(usedUpdateSize);
            UIFixedLength* dbValues = tempAllocator->Allocate<UIFixedLength>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UIFixedLength databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UIFixedLength* current = outputTargets + update.elementId.index;
