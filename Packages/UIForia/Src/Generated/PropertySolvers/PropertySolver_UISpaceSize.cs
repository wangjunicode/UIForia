...
namespace UIForia.Style {
    ...
    internal struct UISpaceSizeWrite {

        public ElementId elementId;
        public UISpaceSize value;

    }

    internal unsafe struct PropertySolverInfo_UISpaceSize {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UISpaceSize* activeTransitionList;
        ...
        public DataList<PropertySolver_UISpaceSize.AnimationValue_UISpaceSize> animationValues;

    }

    internal static unsafe class PropertySolver_UISpaceSize {

        public static void Invoke(ref PropertySolverInfo_UISpaceSize solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UISpaceSize> styleDatabase, UISpaceSize* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UISpaceSize value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UISpaceSize> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UISpaceSizeWrite> writeBuffer = tempAllocator->AllocateList<UISpaceSizeWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UISpaceSize transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UISpaceSize> transitionPrevValues = tempAllocator->AllocateList<UISpaceSize>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UISpaceSize next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UISpaceSize prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUISpaceSize(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UISpaceSize result = UISpaceSize_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UISpaceSize active = new ActiveTransition_UISpaceSize {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UISpaceSize prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UISpaceSize.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUISpaceSize(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UISpaceSize {

            public ElementId elementId;
            public UISpaceSize currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UISpaceSize solverInfo, in SolverParameters parameters, UISpaceSize defaultValue, UISpaceSize* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UISpaceSize* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UISpaceSize* outputTargets, BumpList<UISpaceSizeWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UISpaceSizeWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UISpaceSize solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UISpaceSize transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UISpaceSize> SetupFrameData(ref PropertySolverInfo_UISpaceSize solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UISpaceSize> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UISpaceSize>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UISpaceSize* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UISpaceSizeWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UISpaceSize value = update.Get<UISpaceSize>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UISpaceSizeWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UISpaceSize solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UISpaceSizeWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UISpaceSize.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UISpaceSizeWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UISpaceSize.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UISpaceSize FindInWriteBuffer(ElementId elementId, BumpList<UISpaceSizeWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UISpaceSize FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UISpaceSize> list) {
        ...
        private static UISpaceSize ResolveAnimationVariable(ushort variableNameId, UISpaceSize defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUISpaceSize(out UISpaceSize resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UISpaceSizeWrite> writeBuffer, ElementMap changeMap, UISpaceSize* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UISpaceSize> styleDatabase,
            UISpaceSize* outputTargets,
            ...
            ref BumpList<UISpaceSizeWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UISpaceSize variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UISpaceSize* values = tempAllocator->Allocate<UISpaceSize>(usedUpdateSize);
            UISpaceSize* dbValues = tempAllocator->Allocate<UISpaceSize>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UISpaceSize databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UISpaceSize* current = outputTargets + update.elementId.index;
