    ...
    public unsafe class ElementSystem : IDisposable {
        ...
        public void CleanupFrame() {
            for (int i = 0; i < enabledElementsThisFrame.size; i++) {
                // metaTable[enabledElementsThisFrame[i]].flags ~= (ushort) UIElementFlags.EnableStateChanged;
                // need to sync flags between element instance and table
                // only flags user needs are the enabled set
            }
        }
