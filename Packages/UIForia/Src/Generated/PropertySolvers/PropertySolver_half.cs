...
namespace UIForia.Style {
    ...
    internal unsafe struct PropertySolverInfo_half {
        ...
        public DataList<PropertySolver_half.AnimationValue_half> animationValues;

    }

    internal static unsafe class PropertySolver_half {

        public static void Invoke(ref PropertySolverInfo_half solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<half> styleDatabase, half* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_half value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_half> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<halfWrite> writeBuffer = tempAllocator->AllocateList<halfWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_half transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<half> transitionPrevValues = tempAllocator->AllocateList<half>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        half next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        half prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.Fromhalf(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        half result = half_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_half active = new ActiveTransition_half {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_half prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_half.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.Fromhalf(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_half {

            public ElementId elementId;
            public half currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_half solverInfo, in SolverParameters parameters, half defaultValue, half* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                half* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, half* outputTargets, BumpList<halfWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref halfWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_half solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_half transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_half> SetupFrameData(ref PropertySolverInfo_half solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_half> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_half>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, half* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<halfWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new halfWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_half solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<halfWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_half.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new halfWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_half.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static half FindInWriteBuffer(ElementId elementId, BumpList<halfWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_half FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_half> list) {
        ...
        private static half ResolveAnimationVariable(ushort variableNameId, half defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertTohalf(out half resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<halfWrite> writeBuffer, ElementMap changeMap, half* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            ref BumpList<halfWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                half variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            half* values = tempAllocator->Allocate<half>(usedUpdateSize);
            half* dbValues = tempAllocator->Allocate<half>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                half databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                half* current = outputTargets + update.elementId.index;
