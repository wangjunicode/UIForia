    ...
    public class StandardInstancedRenderer : ElementRenderer {
        ...
        private void RenderBatch(MaterialBatchKey batchKey, BatchEntry[] entries, int start, int end, Vector3 origin, Camera camera) {
            ...
            for (int i = start; i < end; i++) {
                ...
                m_Matrices[instanceId] = Matrix4x4.TRS(origin + data.renderPosition, Quaternion.AngleAxis(style.TransformRotation, Vector3.forward), Vector3.one);
                m_PrimaryColors[instanceId] = style.BackgroundColor;
                m_SecondaryColors[instanceId] = Color.blue;//style.BackgroundColorSecondary;
        ...
        private void SortBatches(RenderData[] drawList, int start, int end) {
            ...
            for (int i = start; i < end; i++) {
                ...
                BackgroundFillType fillType = BackgroundFillType.Gradient;
                BackgroundShapeType shapeType = BackgroundShapeType.Rectangle;
        ...
        private struct MaterialBatch {

            public readonly int key;
            public readonly Material material;
            public readonly LightList<RenderData> renderList;

            public MaterialBatch(int key, Material material) {
                this.key = key;
                this.material = material;
                this.renderList = new LightList<RenderData>();
            }
...
//                m_SizeAndRotations[instanceId] = new Vector4(size.width, size.height, 0, 0);
...
//
//            m_Block.Clear();
//            m_Block.SetVectorArray(s_ColorKey, m_PrimaryColors.List);
//            m_Block.SetVectorArray(s_SizeKey, m_SizeAndRotations.List);
//            m_Block.SetVectorArray(s_ClipRectKey, m_ClipRects.List);
//            m_Block.SetVectorArray(s_BorderRadiusKey, m_BorderRadii.List);
//            m_Block.SetVectorArray(s_BorderSizeKey, m_BorderSizes.List);
//            m_Block.SetVectorArray(s_BorderColorKey, m_BorderColors.List);
//            Graphics.DrawMeshInstanced(s_Mesh, 0, s_InstancedMaterial, m_Matrices.List, count, m_Block, ShadowCastingMode.Off, false, 0, camera, LightProbeUsage.Off);
