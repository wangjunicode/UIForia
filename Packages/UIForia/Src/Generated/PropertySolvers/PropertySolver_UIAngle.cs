...
namespace UIForia.Style {
    ...
    internal struct UIAngleWrite {

        public ElementId elementId;
        public UIAngle value;

    }

    internal unsafe struct PropertySolverInfo_UIAngle {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UIAngle* activeTransitionList;
        ...
        public DataList<PropertySolver_UIAngle.AnimationValue_UIAngle> animationValues;

    }

    internal static unsafe class PropertySolver_UIAngle {

        public static void Invoke(ref PropertySolverInfo_UIAngle solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UIAngle> styleDatabase, UIAngle* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UIAngle value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UIAngle> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UIAngleWrite> writeBuffer = tempAllocator->AllocateList<UIAngleWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIAngle transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UIAngle> transitionPrevValues = tempAllocator->AllocateList<UIAngle>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIAngle next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UIAngle prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUIAngle(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIAngle result = UIAngle_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UIAngle active = new ActiveTransition_UIAngle {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UIAngle prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UIAngle.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUIAngle(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UIAngle {

            public ElementId elementId;
            public UIAngle currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UIAngle solverInfo, in SolverParameters parameters, UIAngle defaultValue, UIAngle* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UIAngle* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UIAngle* outputTargets, BumpList<UIAngleWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UIAngleWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UIAngle solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIAngle transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UIAngle> SetupFrameData(ref PropertySolverInfo_UIAngle solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UIAngle> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UIAngle>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UIAngle* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UIAngleWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UIAngle value = update.Get<UIAngle>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UIAngleWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UIAngle solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UIAngleWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UIAngle.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UIAngleWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UIAngle.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UIAngle FindInWriteBuffer(ElementId elementId, BumpList<UIAngleWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UIAngle FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UIAngle> list) {
        ...
        private static UIAngle ResolveAnimationVariable(ushort variableNameId, UIAngle defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUIAngle(out UIAngle resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UIAngleWrite> writeBuffer, ElementMap changeMap, UIAngle* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UIAngle> styleDatabase,
            UIAngle* outputTargets,
            ...
            ref BumpList<UIAngleWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UIAngle variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UIAngle* values = tempAllocator->Allocate<UIAngle>(usedUpdateSize);
            UIAngle* dbValues = tempAllocator->Allocate<UIAngle>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UIAngle databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UIAngle* current = outputTargets + update.elementId.index;
