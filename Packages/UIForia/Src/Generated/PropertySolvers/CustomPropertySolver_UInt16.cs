...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_UInt16 : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<UInt16>> transitions;
        public UnsafeHashMap<ElementId, UInt16> valueMap;

        public UInt16 defaultValue;

        public UInt16 GetValue(ElementId elementId, UInt16 defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out UInt16 retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<UInt16> dbValues = propertyTables.propertyTable_UInt16;
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<UInt16> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out UInt16 prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<UInt16>() {
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                UInt16 dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<ValueVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<UInt16> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out UInt16 currentValue)) {
                ...
                UInt16 next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                UInt16 prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<UInt16> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out UInt16 currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        UInt16 output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = UInt16_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private UInt16 ResolveAnimationVariable(ushort nextVarId, UInt16 current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<ValueVariable> variableList, ElementId elementId) {
            ...
            for (int v = 0; v < variableList.size; v++) {
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertToUInt16(out resolved)) {
                    continue;
                }
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, UInt16>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<UInt16>>(8, Allocator.Persistent);
        }
