...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_EnumValue : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<EnumValue>> transitions;
        public UnsafeHashMap<ElementId, EnumValue> valueMap;

        public EnumValue defaultValue;

        public EnumValue GetValue(ElementId elementId, EnumValue defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out EnumValue retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<EnumValue> dbValues = propertyTables.propertyTable_EnumValue;
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<EnumValue> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out EnumValue prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<EnumValue>() {
            ...
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                EnumValue value = update.Get<EnumValue>();
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                EnumValue dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<ValueVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<EnumValue> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out EnumValue currentValue)) {
                ...
                EnumValue next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                EnumValue prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
                ...
                EnumValue result = EnumValue_Interpolator.Interpolate(prev, next, keyFrameProp.t);
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<EnumValue> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out EnumValue currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        EnumValue output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = EnumValue_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private EnumValue ResolveAnimationVariable(ushort nextVarId, EnumValue current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<ValueVariable> variableList, ElementId elementId) {
            ...
            for (int v = 0; v < variableList.size; v++) {
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertToEnumValue(out resolved)) {
                    continue;
                }
        ...
        private void ResolveVariables(DataList<PropertyContainer> variableResolveList, CheckedArray<ValueVariable> variableList, CheckedArray<TraversalInfo> traversalInfoTable) {
            ...
            for (int i = 0; i < variableResolveList.size; i++) {
                ...
                valueMap.Add(variableToResolve.elementId, targetIndex == -1
                    ? variableToResolve.Get<EnumValue>()
                    : resolved
                );
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, EnumValue>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<EnumValue>>(8, Allocator.Persistent);
        }
