...
namespace UIForia.Util.Unsafe {
    ...
    /// </summary>
    [JobProducerType(typeof(IJobParallelForDeferBatchExtensions.IJobParallelForDeferBatchedProducer<>))]
    public interface IJobParallelForDeferBatched {
    ...
    public static class IJobParallelForDeferBatchExtensions {
        ...
        internal struct IJobParallelForDeferBatchedProducer<T> where T : struct, IJobParallelForDeferBatched {

            static IntPtr s_JobReflectionData;

            public static unsafe IntPtr Initialize() {
                if (s_JobReflectionData == IntPtr.Zero) {
                    s_JobReflectionData = JobsUtility.CreateJobReflectionData(typeof(T), typeof(T), JobType.ParallelFor, (ExecuteJobFunction) Execute);
                }
        ...
        public static unsafe JobHandle Schedule<T, U>(this T jobData, NativeList<U> list, int innerloopBatchCount, JobHandle dependsOn = new JobHandle()) where T : struct, IJobParallelForDeferBatched where U : struct {
            void* atomicSafetyHandlePtr = null;
            var scheduleParams = new JobsUtility.JobScheduleParameters(UnsafeUtility.AddressOf(ref jobData), IJobParallelForDeferBatchedProducer<T>.Initialize(), dependsOn, ScheduleMode.Batched);
        ...
        public static unsafe JobHandle Schedule<T>(this T jobData, int* forEachCount, int innerloopBatchCount,
            ...
            where T : struct, IJobParallelForDeferBatched {
            var scheduleParams = new JobsUtility.JobScheduleParameters(
                UnsafeUtility.AddressOf(ref jobData),
                IJobParallelForDeferBatchedProducer<T>.Initialize(), dependsOn, ScheduleMode.Batched);
