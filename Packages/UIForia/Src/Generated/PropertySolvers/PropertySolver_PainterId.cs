...
namespace UIForia.Style {
    ...
    internal struct PainterIdWrite {

        public ElementId elementId;
        public PainterId value;

    }

    internal unsafe struct PropertySolverInfo_PainterId {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_PainterId* activeTransitionList;
        ...
        public DataList<PropertySolver_PainterId.AnimationValue_PainterId> animationValues;

    }

    internal static unsafe class PropertySolver_PainterId {

        public static void Invoke(ref PropertySolverInfo_PainterId solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<PainterId> styleDatabase, PainterId* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_PainterId value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_PainterId> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<PainterIdWrite> writeBuffer = tempAllocator->AllocateList<PainterIdWrite>(maxWriteBufferSize);
            CheckedArray<PainterVariableType> variables = SolverParameters.GetVariableList_PainterVariableType(parameters);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_PainterId transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<PainterId> transitionPrevValues = tempAllocator->AllocateList<PainterId>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        PainterId next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        PainterId prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromPainterId(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        PainterId result = PainterId_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_PainterId active = new ActiveTransition_PainterId {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_PainterId prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_PainterId.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromPainterId(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_PainterId {

            public ElementId elementId;
            public PainterId currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_PainterId solverInfo, in SolverParameters parameters, PainterId defaultValue, PainterId* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                PainterId* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, PainterId* outputTargets, BumpList<PainterIdWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref PainterIdWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_PainterId solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_PainterId transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_PainterId> SetupFrameData(ref PropertySolverInfo_PainterId solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_PainterId> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_PainterId>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, PainterId* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<PainterIdWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                PainterId value = update.Get<PainterId>();
                ...
                writeBuffer.array[writeBuffer.size++] = new PainterIdWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_PainterId solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<PainterIdWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_PainterId.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new PainterIdWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_PainterId.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static PainterId FindInWriteBuffer(ElementId elementId, BumpList<PainterIdWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_PainterId FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_PainterId> list) {
        ...
        private static PainterId ResolveAnimationVariable(ushort variableNameId, PainterId defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<PainterVariableType> variables, ElementId elementId) {
            ...
            for (int i = 0; i < variables.size; i++) {
                PainterVariableType variable = variables[i];
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToPainterId(out PainterId resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<PainterVariableType> variables, ref BumpList<PainterIdWrite> writeBuffer, ElementMap changeMap, PainterId* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<PainterId> styleDatabase,
            PainterId* outputTargets,
            ...
            ref BumpList<PainterIdWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                PainterId variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            PainterId* values = tempAllocator->Allocate<PainterId>(usedUpdateSize);
            PainterId* dbValues = tempAllocator->Allocate<PainterId>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                PainterId databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                PainterId* current = outputTargets + update.elementId.index;
