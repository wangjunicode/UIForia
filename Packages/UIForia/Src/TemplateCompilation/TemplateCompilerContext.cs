...
namespace UIForia.Compilers {
    ...
    public class TemplateCompilerContext {
        ...
        public TemplateCompilerContext(Diagnostics diagnostics) {
            ...
            for (int i = 0; i < variables.size; i++) {
                if (variables[i].Name == name) {
                    if (variables[i].Type != type) {
                        diagnostics.LogError("Variable already taken: " + name);
                        return null;
                    }
        ...
        public LambdaExpression Build(string debugFnName) {
            return Expression.Lambda(Expression.Block(returnType, variables.CloneArray(), statements.ToArray()), debugFnName, false, parameters);
        }
