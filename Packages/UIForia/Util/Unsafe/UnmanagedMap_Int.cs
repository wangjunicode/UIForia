...
namespace UIForia.Util.Unsafe {
    ...
    public unsafe struct UnmanagedIntMap<TValue> : IDisposable where TValue : unmanaged {

        [NativeDisableUnsafePtrRestriction]
        private readonly UntypedIntMap* mapState;

        public UnmanagedIntMap(UntypedIntMap* mapState) {
            this.mapState = mapState->GetItemSize() == sizeof(TValue)
                ? mapState
                : default;
        }

        public UnmanagedIntMap(int initialCapacity, Allocator allocator, float fillFactor = 0.75f) {
    ...
    public unsafe struct UntypedIntMap : IDisposable {
        ...
        public T Add<T>(int key, in T value) where T : unmanaged {
            if (key == k_FreeKey) {
