...
//            }
        ...
        public static StructList<ExpressionToken> Tokenize(string input,  StructList<ExpressionToken> retn = null, TokenizerContext context = null) {
            StructList<ExpressionToken> output = retn ?? new StructList<ExpressionToken>();

            if (context == null) {
                context = new TokenizerContext(input);
            }
            else {
                context.Reset(input);
            }
            ...
            while (context.ptr < input.Length) {
                ...
                TryReadStringInterpolation(context, output);
                ...
                TryReadCharacters(context, "{", ExpressionTokenType.BracketOpen, output);
                TryReadCharacters(context, "}", ExpressionTokenType.BracketClose, output);
        ...
        private static unsafe void TryReadStringInterpolation(TokenizerContext context, StructList<ExpressionToken> output) {

            if (context.ptr + 3 >= context.input.Length) {
                return;
            }
            ...
            fixed (char* cbuffer = context.input) {

                CharStream stream = new CharStream(cbuffer, context.ptr + 1, context.input.Length);
