...
namespace UIForia.ListTypes {
    ...
    [DebuggerTypeProxy(typeof(DebugView_GridPlacement))]
    internal unsafe struct List_GridPlacement : IBasicList<GridPlacement> {

        public int size;
        private ushort capacityShiftBits;
        private AllocatorUShort allocator;

        [NativeDisableUnsafePtrRestriction] public GridPlacement* array;

        private const int k_MinCapacity = 4;

        public List_GridPlacement(int initialCapacity, Allocator allocator, bool clearMemory = false) {
        ...
        private void Initialize(int capacity, Allocator allocator, bool clearMemory) {
            ...
            this.array = (GridPlacement*) UnsafeUtility.Malloc(sizeof(GridPlacement) * capacity, UnsafeUtility.AlignOf<GridPlacement>(), allocator);
            this.capacityShiftBits = (ushort) BitUtil.GetPowerOfTwoBitIndex((uint) capacity);
            if (clearMemory) {
                UnsafeUtility.MemClear(array, sizeof(GridPlacement) * capacity);
            }
        }

        public ref GridPlacement this[int index] {
            get => ref array[index];
        }
        ...
        public void Add(in GridPlacement item) {
        ...
        public void AddUnchecked(in GridPlacement item) {
            array[size++] = item;
        }

        public void AddRange(GridPlacement* items, int itemCount) {
        ...
        public void EnsureCapacity(int desiredCapacity, bool clearMemory = false) {
            ...
            long bytesToMalloc = sizeof(GridPlacement) * capacity;
            void* newPointer = UnsafeUtility.Malloc(bytesToMalloc, UnsafeUtility.AlignOf<GridPlacement>(), fullAllocator);
            ...
            if (array != default) {
                int bytesToCopy = size * sizeof(GridPlacement);
            ...
            array = (GridPlacement*) newPointer;
        }
        ...
        public ref GridPlacement Get(int index) {
            return ref array[index];
        }

        public void Set(in GridPlacement item, int index) {
            array[index] = item;
        }
        ...
        public GridPlacement* GetPointer(int index) {
            return array + index;
        }
        ...
        public GridPlacement GetLast() {
            return array[size - 1];
        }

        public void CopyFrom(GridPlacement* data, int count) {
            EnsureCapacity(count);
            size = count;
            TypedUnsafe.MemCpy(array, data, size);
        }

        public void CopyFrom(GridPlacement* data, int count, Allocator allocator) {
            if (array == null) {
        ...
        public struct Shared : IDisposable {
            ...
            [NativeDisableUnsafePtrRestriction] public List_GridPlacement* state;

            public Shared(int initialCapacity, Allocator allocator, bool clear = false) {
                this.state = TypedUnsafe.Malloc<List_GridPlacement>(1, allocator);
                *this.state = new List_GridPlacement(initialCapacity, allocator, clear);
            }
            ...
            public ref GridPlacement GetReference(int index) {
            ...
            public GridPlacement* GetArrayPointer() {
                return state->array;
            }
            ...
            public void FilterSwapRemove<TFilter>(TFilter filter) where TFilter : IListFilter<GridPlacement> {
        ...
        public struct DebugView_GridPlacement {

            public int size;
            public int capacity;
            public GridPlacement[] data;

            public DebugView_GridPlacement(List_GridPlacement target) {
                this.size = target.size;
                this.capacity = target.Capacity;
                this.data = new GridPlacement[size];
                for (int i = 0; i < size; i++) {
                    data[i] = target[i];
                }
            }

            public DebugView_GridPlacement(List_GridPlacement.Shared target) {
                this.size = target.size;
                this.capacity = target.capacity;
                this.data = new GridPlacement[size];
                for (int i = 0; i < size; i++) {
