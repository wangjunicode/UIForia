...
namespace UIForia.Util.Unsafe {
    ...
    public unsafe struct UntypedIntMap : IDisposable {
        ...
        /** Keys and values */
        [NativeDisableUnsafePtrRestriction] private byte* data;
        ...
        public static UntypedIntMap* Create<T>(int initialCapacity, float fillFactor, Allocator allocator) where T : unmanaged {
            ...
            *retn = default; // clear for safety
            ...
            retn->data = (byte*) UnsafeUtility.Malloc(byteCount, UnsafeUtility.AlignOf<int>(), allocator);
            UnsafeUtility.MemClear(retn->data, byteCount);
            retn->size = 0;
        ...
        public void Clear<T>() where T : unmanaged {
            long byteCount = itemSize * capacity;
            UnsafeUtility.MemClear(data, byteCount);
        }
        ...
        public bool TryGetPointer<T>(int key, out T* value) where T : unmanaged {
            ...
            int* currentCell = (int*) (data + (idx * itemSize));
            ...
            while (true) {
                ...
                currentCell = (int*) (data + (idx * itemSize));
                if (*currentCell == k_FreeKey) {
                    value = default;
                    return false;
                }
        ...
        private void ShiftKeys<T>(uint pos) where T : unmanaged {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    int* k = (int*) (data + (pos * itemSize));

                    if (*k == k_FreeKey) {
                        int* pLast = (int*) (data + (last * itemSize));
                        *pLast = k_FreeKey;
                        return;
                    }
                ...
                int* pLastIndex = (int*) (data + (last * itemSize));
                int* pPosIndex = (int*) (data + (pos * itemSize));
        ...
        private void Rehash<T>() where T : unmanaged {
            ...
            data = (byte*) UnsafeUtility.Malloc(capacity * itemSize, UnsafeUtility.AlignOf<int>(), allocator);

            UnsafeUtility.MemClear(data, capacity * itemSize);
