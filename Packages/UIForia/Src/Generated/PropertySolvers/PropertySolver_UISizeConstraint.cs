...
namespace UIForia.Style {
    ...
    internal struct UISizeConstraintWrite {

        public ElementId elementId;
        public UISizeConstraint value;

    }

    internal unsafe struct PropertySolverInfo_UISizeConstraint {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UISizeConstraint* activeTransitionList;
        ...
        public DataList<PropertySolver_UISizeConstraint.AnimationValue_UISizeConstraint> animationValues;

    }

    internal static unsafe class PropertySolver_UISizeConstraint {

        public static void Invoke(ref PropertySolverInfo_UISizeConstraint solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UISizeConstraint> styleDatabase, UISizeConstraint* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UISizeConstraint value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UISizeConstraint> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UISizeConstraintWrite> writeBuffer = tempAllocator->AllocateList<UISizeConstraintWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UISizeConstraint transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UISizeConstraint> transitionPrevValues = tempAllocator->AllocateList<UISizeConstraint>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UISizeConstraint next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UISizeConstraint prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUISizeConstraint(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UISizeConstraint result = UISizeConstraint_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UISizeConstraint active = new ActiveTransition_UISizeConstraint {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UISizeConstraint prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UISizeConstraint.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUISizeConstraint(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UISizeConstraint {

            public ElementId elementId;
            public UISizeConstraint currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UISizeConstraint solverInfo, in SolverParameters parameters, UISizeConstraint defaultValue, UISizeConstraint* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UISizeConstraint* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UISizeConstraint* outputTargets, BumpList<UISizeConstraintWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UISizeConstraintWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UISizeConstraint solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UISizeConstraint transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UISizeConstraint> SetupFrameData(ref PropertySolverInfo_UISizeConstraint solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UISizeConstraint> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UISizeConstraint>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UISizeConstraint* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UISizeConstraintWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UISizeConstraint value = update.Get<UISizeConstraint>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UISizeConstraintWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UISizeConstraint solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UISizeConstraintWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UISizeConstraint.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UISizeConstraintWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UISizeConstraint.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UISizeConstraint FindInWriteBuffer(ElementId elementId, BumpList<UISizeConstraintWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UISizeConstraint FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UISizeConstraint> list) {
        ...
        private static UISizeConstraint ResolveAnimationVariable(ushort variableNameId, UISizeConstraint defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            UISizeConstraint value = default;
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUISizeConstraint(out UISizeConstraint resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UISizeConstraintWrite> writeBuffer, ElementMap changeMap, UISizeConstraint* outputTargets) {
            ...
            for (int updateIdx = 0; updateIdx < propertyList.size; updateIdx++) {
                ...
                if (start == -1) {
                    ...
                    writeBuffer.array[writeBuffer.size++] = new UISizeConstraintWrite() {
                        elementId = update.elementId,
                        value = update.Get<UISizeConstraint>(),
                    };
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UISizeConstraint> styleDatabase,
            UISizeConstraint* outputTargets,
            ...
            ref BumpList<UISizeConstraintWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UISizeConstraint variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UISizeConstraint* values = tempAllocator->Allocate<UISizeConstraint>(usedUpdateSize);
            UISizeConstraint* dbValues = tempAllocator->Allocate<UISizeConstraint>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UISizeConstraint databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UISizeConstraint* current = outputTargets + update.elementId.index;
