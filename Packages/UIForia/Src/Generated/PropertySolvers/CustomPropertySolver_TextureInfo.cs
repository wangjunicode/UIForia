...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_TextureInfo : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<TextureInfo>> transitions;
        public UnsafeHashMap<ElementId, TextureInfo> valueMap;

        public TextureInfo defaultValue;

        public TextureInfo GetValue(ElementId elementId, TextureInfo defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out TextureInfo retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<TextureInfo> dbValues = propertyTables.propertyTable_TextureInfo;
            ...
            CheckedArray<TextureVariable> variableList = sharedSolverInfo.GetVariableList_TextureVariable();
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<TextureInfo> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out TextureInfo prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<TextureInfo>() {
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                TextureInfo dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<TextureVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<TextureInfo> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out TextureInfo currentValue)) {
                ...
                TextureInfo next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                TextureInfo prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<TextureInfo> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out TextureInfo currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        TextureInfo output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = TextureInfo_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private TextureInfo ResolveAnimationVariable(ushort nextVarId, TextureInfo current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<TextureVariable> variableList, ElementId elementId) {
            ...
            int targetIndex = -1;
            TextureInfo resolved = default;

            for (int v = 0; v < variableList.size; v++) {

                ref TextureVariable variable = ref variableList.Get(v);
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertToTextureInfo(out resolved)) {
                    continue;
                }
        ...
        private void ResolveVariables(DataList<PropertyContainer> variableResolveList, CheckedArray<TextureVariable> variableList, CheckedArray<TraversalInfo> traversalInfoTable) {
            ...
            for (int i = 0; i < variableResolveList.size; i++) {
                ...
                valueMap.Add(variableToResolve.elementId, targetIndex == -1
                    ? variableToResolve.Get<TextureInfo>()
                    : resolved
                );
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, TextureInfo>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<TextureInfo>>(8, Allocator.Persistent);
        }
