...
namespace UIForia.Style {
    ...
    internal struct FontAssetIdWrite {

        public ElementId elementId;
        public FontAssetId value;

    }

    internal unsafe struct PropertySolverInfo_FontAssetId {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_FontAssetId* activeTransitionList;
        ...
        public DataList<PropertySolver_FontAssetId.AnimationValue_FontAssetId> animationValues;

    }

    internal static unsafe class PropertySolver_FontAssetId {

        public static void Invoke(ref PropertySolverInfo_FontAssetId solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<FontAssetId> styleDatabase, FontAssetId* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_FontAssetId value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_FontAssetId> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<FontAssetIdWrite> writeBuffer = tempAllocator->AllocateList<FontAssetIdWrite>(maxWriteBufferSize);
            CheckedArray<FontAssetVariable> variables = SolverParameters.GetVariableList_FontAssetVariable(parameters);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_FontAssetId transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<FontAssetId> transitionPrevValues = tempAllocator->AllocateList<FontAssetId>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        FontAssetId next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        FontAssetId prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromFontAssetId(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        FontAssetId result = FontAssetId_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_FontAssetId active = new ActiveTransition_FontAssetId {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_FontAssetId prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_FontAssetId.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromFontAssetId(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_FontAssetId {

            public ElementId elementId;
            public FontAssetId currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_FontAssetId solverInfo, in SolverParameters parameters, FontAssetId defaultValue, FontAssetId* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                FontAssetId* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, FontAssetId* outputTargets, BumpList<FontAssetIdWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref FontAssetIdWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_FontAssetId solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_FontAssetId transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_FontAssetId> SetupFrameData(ref PropertySolverInfo_FontAssetId solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_FontAssetId> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_FontAssetId>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, FontAssetId* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<FontAssetIdWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                FontAssetId value = update.Get<FontAssetId>();
                ...
                writeBuffer.array[writeBuffer.size++] = new FontAssetIdWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_FontAssetId solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<FontAssetIdWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_FontAssetId.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new FontAssetIdWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_FontAssetId.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static FontAssetId FindInWriteBuffer(ElementId elementId, BumpList<FontAssetIdWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_FontAssetId FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_FontAssetId> list) {
        ...
        private static FontAssetId ResolveAnimationVariable(ushort variableNameId, FontAssetId defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<FontAssetVariable> variables, ElementId elementId) {
            ...
            for (int i = 0; i < variables.size; i++) {
                FontAssetVariable variable = variables[i];
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToFontAssetId(out FontAssetId resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<FontAssetVariable> variables, ref BumpList<FontAssetIdWrite> writeBuffer, ElementMap changeMap, FontAssetId* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<FontAssetId> styleDatabase,
            FontAssetId* outputTargets,
            ...
            ref BumpList<FontAssetIdWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                FontAssetId variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            FontAssetId* values = tempAllocator->Allocate<FontAssetId>(usedUpdateSize);
            FontAssetId* dbValues = tempAllocator->Allocate<FontAssetId>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                FontAssetId databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                FontAssetId* current = outputTargets + update.elementId.index;
