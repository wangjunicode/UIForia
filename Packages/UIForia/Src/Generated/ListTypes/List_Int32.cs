...
namespace UIForia.ListTypes {
    ...
    [DebuggerTypeProxy(typeof(DebugView_Int32))]
    internal unsafe struct List_Int32 : IBasicList<Int32> {

        public int size;
        private ushort capacityShiftBits;
        private AllocatorUShort allocator;

        [NativeDisableUnsafePtrRestriction] public Int32* array;

        private const int k_MinCapacity = 4;

        public List_Int32(int initialCapacity, Allocator allocator, bool clearMemory = false) {
        ...
        private void Initialize(int capacity, Allocator allocator, bool clearMemory) {
            ...
            this.array = (Int32*) UnsafeUtility.Malloc(sizeof(Int32) * capacity, UnsafeUtility.AlignOf<Int32>(), allocator);
            this.capacityShiftBits = (ushort) BitUtil.GetPowerOfTwoBitIndex((uint) capacity);
            if (clearMemory) {
                UnsafeUtility.MemClear(array, sizeof(Int32) * capacity);
            }
        }
        ...
        public void AddUnchecked(in Int32 item) {
            array[size++] = item;
        }

        public void AddRange(Int32* items, int itemCount) {
        ...
        public void EnsureCapacity(int desiredCapacity, bool clearMemory = false) {
            ...
            long bytesToMalloc = sizeof(Int32) * capacity;
            void* newPointer = UnsafeUtility.Malloc(bytesToMalloc, UnsafeUtility.AlignOf<Int32>(), fullAllocator);
            ...
            if (array != default) {
                int bytesToCopy = size * sizeof(Int32);
        ...
        public ref Int32 Get(int index) {
            return ref array[index];
        }

        public void Set(in Int32 item, int index) {
            array[index] = item;
        }
        ...
        public Int32* GetPointer(int index) {
            return array + index;
        }
        ...
        public Int32 GetLast() {
            return array[size - 1];
        }

        public void CopyFrom(Int32* data, int count) {
            EnsureCapacity(count);
            size = count;
            TypedUnsafe.MemCpy(array, data, size);
        }

        public void CopyFrom(Int32* data, int count, Allocator allocator) {
            if (array == null) {
        ...
        public struct Shared : IDisposable {
            ...
            [NativeDisableUnsafePtrRestriction] public List_Int32* state;

            public Shared(int initialCapacity, Allocator allocator, bool clear = false) {
                this.state = TypedUnsafe.Malloc<List_Int32>(1, allocator);
                *this.state = new List_Int32(initialCapacity, allocator, clear);
            }
            ...
            public ref Int32 GetReference(int index) {
            ...
            public void FilterSwapRemove<TFilter>(TFilter filter) where TFilter : IListFilter<Int32> {
        ...
        public struct DebugView_Int32 {

            public int size;
            public int capacity;
            public Int32[] data;

            public DebugView_Int32(List_Int32 target) {
                this.size = target.size;
                this.capacity = target.Capacity;
                this.data = new Int32[size];
                for (int i = 0; i < size; i++) {
                    data[i] = target[i];
                }
            }

            public DebugView_Int32(List_Int32.Shared target) {
                this.size = target.size;
                this.capacity = target.capacity;
                this.data = new Int32[size];
                for (int i = 0; i < size; i++) {
