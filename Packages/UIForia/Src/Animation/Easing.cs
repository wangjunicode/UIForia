...
namespace UIForia.Rendering {
    ...
    public static class Easing {
        ...
        private const float PiOverTwo = math.PI / 2.0f;

        /// <summary>
        /// Interpolate using the specified function.
        /// </summary>
        public static float Interpolate(float p, EasingFunction easingFunction, ref Bezier bezier) {
            switch (easingFunction) {
                ...
                case EasingFunction.Bezier: return bezier.Evaluate(p);
        ...
        public static float ElasticEaseInOut(float p) {
            if (p < 0.5f) {
                return 0.5f * math.sin(13 * PiOverTwo * (2 * p)) * math.pow(2, 10 * ((2 * p) - 1));
            }
            else {
                return 0.5f * (math.sin(-13 * PiOverTwo * ((2 * p - 1) + 1)) * math.pow(2, -10 * (2 * p - 1)) + 2);
            }
        }
