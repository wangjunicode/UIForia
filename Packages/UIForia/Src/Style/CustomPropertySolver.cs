...
namespace UIForia.Style {
    ...
    // internal unsafe struct CustomPropertySolver_Float : ICustomPropertySolver {
    //
    //     public DataList<PropertyUpdate> sharedProperties;
    //     public DataList<Transition<float>> transitions;
    //     public UnsafeHashMap<ElementId, float> valueMap;
    //
    //     public float defaultValue;
    //
    //     public float GetValue(ElementId elementId, float defaultOverride, bool useDefault) {
    //
    //         if (!valueMap.TryGetValue(elementId, out float retn)) {
    ...
    //
    //         DataList<float> dbValues = propertyTables.propertyTable_float;
    //
    ...
    //                     ref Transition<float> transition = ref transitions[j];
    //
    ...
    //
    //                     if (!valueMap.TryGetValue(elementId, out float prevValue)) {
    //                         prevValue = defaultValue;
    //                     }
    //
    //                     TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];
    //
    //                     transitions.Add(new Transition<float>() {
    ...
    //             }
    //
    //             float dbValue = dbValues[update.dbValueLocation];
    //
    ...
    //         }
    //
    //         ResolveVariables(variableResolveList, sharedSolverInfo.valueVariableList, parameters.traversalTable);
    //
    //         variableResolveList.size = 0;
    //
    //         ApplyAnimations(ref parameters, sharedSolverInfo, keyFramesProperties, dbValues, defaultIndex, animationMap);
    //
    //         ApplyTransitions(ref parameters, animationMap);
    //
    //     }
    //
    //     private void ApplyAnimations(ref SolverParameters parameters, CustomSolverInfo sharedSolverInfo, DataList<KeyFrameResult> keyFramesProperties, DataList<float> dbValues, int defaultIndex, ElementMap animationMap) {
    ...
    //
    //             if (!valueMap.TryGetValue(keyFrameProp.elementId, out float currentValue)) {
    ...
    //             float next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
    //             float prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
    //
    //             next = ResolveAnimationVariable(keyFrameProp.nextVarId, next, parameters.traversalTable, sharedSolverInfo.valueVariableList, keyFrameProp.elementId);
    //             prev = ResolveAnimationVariable(keyFrameProp.prevVarId, prev, parameters.traversalTable, sharedSolverInfo.valueVariableList, keyFrameProp.elementId);
    //
    //             float result = float_Interpolator.Interpolate(prev, next, keyFrameProp.t);
    //
    ...
    //         for (int i = 0; i < transitions.size; i++) {
    //
    //             ref Transition<float> transition = ref transitions[i];
    //
    //             if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
    //                 continue;
    //             }
    //
    //             if (!valueMap.TryGetValue(transition.elementId, out float currentValue)) {
    ...
    //                     float progress = 0;
    //                     float output = transition.prevValue;
    //
    ...
    //                         output = float_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
    //                     }
    //
    //                     if (progress == 1f) {
    //                         transition.state = TransitionState.Pending;
    ...
    //     }
    //
    //     private float ResolveAnimationVariable(ushort nextVarId, float current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<ValueVariable> variableList, ElementId elementId) {
    //
    ...
    //
    //             if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertToFloat(out resolved)) {
    //                 continue;
    //             }
    //
    ...
    //         valueMap = new UnsafeHashMap<ElementId, float>(31, Allocator.Persistent);
    //         sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
    //         transitions = new DataList<Transition<float>>(8, Allocator.Persistent);
    //     }
    //
    //     public void Dispose() {
    //         valueMap.Dispose();
