...
namespace UIForia.Style {
    ...
    internal unsafe struct PropertySolverInfo_UInt16 {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UInt16* activeTransitionList;
        ...
        public DataList<PropertySolver_UInt16.AnimationValue_UInt16> animationValues;

    }

    internal static unsafe class PropertySolver_UInt16 {

        public static void Invoke(ref PropertySolverInfo_UInt16 solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UInt16> styleDatabase, UInt16* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UInt16 value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UInt16> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UInt16Write> writeBuffer = tempAllocator->AllocateList<UInt16Write>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UInt16 transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UInt16> transitionPrevValues = tempAllocator->AllocateList<UInt16>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UInt16 next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UInt16 prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUInt16(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UInt16 result = UInt16_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UInt16 active = new ActiveTransition_UInt16 {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UInt16 prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UInt16.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUInt16(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UInt16 {
        ...
        private static void WriteDefaultValues(in PropertySolverInfo_UInt16 solverInfo, in SolverParameters parameters, UInt16 defaultValue, UInt16* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UInt16* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UInt16* outputTargets, BumpList<UInt16Write> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UInt16Write entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UInt16 solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UInt16 transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UInt16> SetupFrameData(ref PropertySolverInfo_UInt16 solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UInt16> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UInt16>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UInt16* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UInt16Write> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UInt16 value = update.Get<UInt16>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UInt16Write() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UInt16 solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UInt16Write> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UInt16.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UInt16Write() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UInt16.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UInt16 FindInWriteBuffer(ElementId elementId, BumpList<UInt16Write> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UInt16 FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UInt16> list) {
        ...
        private static UInt16 ResolveAnimationVariable(ushort variableNameId, UInt16 defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUInt16(out UInt16 resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UInt16Write> writeBuffer, ElementMap changeMap, UInt16* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UInt16> styleDatabase,
            UInt16* outputTargets,
            ...
            ref BumpList<UInt16Write> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UInt16 variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UInt16* values = tempAllocator->Allocate<UInt16>(usedUpdateSize);
            UInt16* dbValues = tempAllocator->Allocate<UInt16>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UInt16 databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UInt16* current = outputTargets + update.elementId.index;
