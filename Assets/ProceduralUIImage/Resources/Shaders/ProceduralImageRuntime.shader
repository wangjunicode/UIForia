	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f IN) : SV_Target
			{
                ...
                float f = l - distance(v, l);
                
                if(f < 0) {
                    // f < 0 means we are in the content section (if we have a non 0 border width)
                    // get andres to help me make this a smooth transition
                    return fixed4(1, 0, 0, 1); //lerp(color, fixed4(1, 0, 0, 1), 1 - f); 
                }	
