...
namespace UIForia.Parsing.Expressions.Tokenizer {
    ...
    public struct TokenStream {
        ...
        private StructStack<int> stack;
        internal StructList<ExpressionToken> tokens;

        public TokenStream(StructList<ExpressionToken> tokens) {
            this.ptr = 0;
            this.tokens = tokens;
            this.lastTokenIndex = tokens.Count;
            this.stack = StructStack<int>.Get();
        }
        ...
        public ExpressionToken Last {
            [DebuggerStepThrough] get { return (tokens.size == 0) ? ExpressionToken.Invalid : tokens[tokens.size - 1]; }
        }
        ...
        public void Advance(int count = 1) {
            ptr = Mathf.Min(ptr + count, tokens.size);
        }

        [DebuggerStepThrough]
        public void Set(int idx) {
            ptr = Mathf.Min(idx, tokens.size);
        }
        ...
        public TokenStream Slice(RangeInt range) {
            return new TokenStream(tokens.Slice(range));
        }
