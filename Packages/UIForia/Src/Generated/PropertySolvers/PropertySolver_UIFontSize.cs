...
namespace UIForia.Style {
    ...
    internal struct UIFontSizeWrite {

        public ElementId elementId;
        public UIFontSize value;

    }

    internal unsafe struct PropertySolverInfo_UIFontSize {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UIFontSize* activeTransitionList;
        ...
        public DataList<PropertySolver_UIFontSize.AnimationValue_UIFontSize> animationValues;

    }

    internal static unsafe class PropertySolver_UIFontSize {

        public static void Invoke(ref PropertySolverInfo_UIFontSize solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UIFontSize> styleDatabase, UIFontSize* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UIFontSize value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UIFontSize> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UIFontSizeWrite> writeBuffer = tempAllocator->AllocateList<UIFontSizeWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIFontSize transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UIFontSize> transitionPrevValues = tempAllocator->AllocateList<UIFontSize>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIFontSize next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UIFontSize prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUIFontSize(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIFontSize result = UIFontSize_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UIFontSize active = new ActiveTransition_UIFontSize {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UIFontSize prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UIFontSize.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUIFontSize(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UIFontSize {

            public ElementId elementId;
            public UIFontSize currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UIFontSize solverInfo, in SolverParameters parameters, UIFontSize defaultValue, UIFontSize* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UIFontSize* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UIFontSize* outputTargets, BumpList<UIFontSizeWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UIFontSizeWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UIFontSize solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIFontSize transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UIFontSize> SetupFrameData(ref PropertySolverInfo_UIFontSize solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UIFontSize> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UIFontSize>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UIFontSize* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UIFontSizeWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UIFontSize value = update.Get<UIFontSize>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UIFontSizeWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UIFontSize solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UIFontSizeWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UIFontSize.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UIFontSizeWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UIFontSize.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UIFontSize FindInWriteBuffer(ElementId elementId, BumpList<UIFontSizeWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UIFontSize FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UIFontSize> list) {
        ...
        private static UIFontSize ResolveAnimationVariable(ushort variableNameId, UIFontSize defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUIFontSize(out UIFontSize resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UIFontSizeWrite> writeBuffer, ElementMap changeMap, UIFontSize* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UIFontSize> styleDatabase,
            UIFontSize* outputTargets,
            ...
            ref BumpList<UIFontSizeWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UIFontSize variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UIFontSize* values = tempAllocator->Allocate<UIFontSize>(usedUpdateSize);
            UIFontSize* dbValues = tempAllocator->Allocate<UIFontSize>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UIFontSize databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UIFontSize* current = outputTargets + update.elementId.index;
