...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_UIColor : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<UIColor>> transitions;
        public UnsafeHashMap<ElementId, UIColor> valueMap;

        public UIColor defaultValue;

        public UIColor GetValue(ElementId elementId, UIColor defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out UIColor retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<UIColor> dbValues = propertyTables.propertyTable_UIColor;
            ...
            CheckedArray<ColorVariable> variableList = sharedSolverInfo.GetVariableList_ColorVariable();
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<UIColor> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out UIColor prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<UIColor>() {
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                UIColor dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<ColorVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<UIColor> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out UIColor currentValue)) {
                ...
                UIColor next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                UIColor prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<UIColor> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out UIColor currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        UIColor output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = UIColor_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private UIColor ResolveAnimationVariable(ushort nextVarId, UIColor current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<ColorVariable> variableList, ElementId elementId) {
            ...
            int targetIndex = -1;
            UIColor resolved = default;

            for (int v = 0; v < variableList.size; v++) {

                ref ColorVariable variable = ref variableList.Get(v);
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertToUIColor(out resolved)) {
                    continue;
                }
        ...
        private void ResolveVariables(DataList<PropertyContainer> variableResolveList, CheckedArray<ColorVariable> variableList, CheckedArray<TraversalInfo> traversalInfoTable) {
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, UIColor>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<UIColor>>(8, Allocator.Persistent);
        }
