...
namespace UIForia.Text {
    ...
    public class TextInfoOld {

        private static int s_SpanIdGenerator;

        internal StructList<TextLineInfo> lineInfoList;
        ...
        public TextInfoOld(string content, in SVGXTextStyle style = default, bool inheritStyleProperties = false) {
            this.rootSpan = new TextSpan();
            this.rootSpan.textInfoOld = this;
            this.spanList = new LightList<TextSpan>();
            this.lineInfoList = StructList<TextLineInfo>.Get();
        ...
        public Size Layout(Vector2 offset, float width) {
            ...
            if (requiresLayout) {
                ...
                TextLineInfo lastLine = lineInfoList[lineInfoList.size - 1];
                float maxWidth = 0;

                TextLineInfo[] lineInfos = lineInfoList.array;
        ...
        private void ApplyLineAndWordOffsets(float totalWidth, TextAlignment alignment, int lineStart = 0) {
            ...
            TextLineInfo[] lines = lineInfoList.array;
            ...
            for (int lineIndex = lineStart; lineIndex < lineCount; lineIndex++) {
                int spanStart = 0; // lines[lineIndex].spanStart;
                int spanEnd = 0; //lines[lineIndex].spanEnd;
        ...
        private StructList<TextLineInfo> RunLayout(StructList<TextLineInfo> lines, float width) {
            lines.size = 0;

            TextLineInfo currentLine = new TextLineInfo();
            ...
            TextLineInfo[] linearray = lines.array;
            for (int i = 0; i < lines.size; i++) {
                // ref TextLineInfo lineInfo = ref linearray[i];
                // lineInfo.spanEnd++;
                // // align via baseline
        ...
        private void UpdateIntrinsics() {
            ...
            StructList<TextLineInfo> lines = StructList<TextLineInfo>.Get();
            ...
            if (lines.size == 0) {
                ...
                StructList<TextLineInfo>.Release(ref lines);
                return;
            }
        ...
        private int GetPreviousWordEdge(int characterIdx) {
            ...
            TextLineInfo lineInfo = lineInfoList.array[lineIndex];
        ...
        public Vector2 GetCursorPosition(int cursorIndex) {
            ...
            TextLineInfo lineInfo = lineInfoList.array[charInfo.lineIndex];
        ...
        public Rect GetLineRectFromCharacterIndex(int characterIdx) {
            ...
            TextLineInfo lineInfo = lineInfoList[lineIndex];
        ...
        public SelectionRange SelectWordAtPoint(Vector2 point) {
            ...
            int spanIndex = -1;
            float closestDistance = float.MaxValue;
            TextLineInfo line = lineInfoList.array[nearestLine];
