...
namespace UIForia {
    ...
    ///
    /// The kink here is that we also need to support AttributeStartsWith/Contains/EndsWith. The solution here is to
    /// keep an indexed list of 'burstable' string values (a StringHandle) which is a pointer to the string converted
    /// to ushort (which is the size of a character in C#, 2 bytes)
    ///
    ...
    /// Remove the string and release its memory, then recycle the index.
    /// </summary>
