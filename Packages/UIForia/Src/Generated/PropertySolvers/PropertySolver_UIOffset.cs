...
namespace UIForia.Style {
    ...
    internal struct UIOffsetWrite {

        public ElementId elementId;
        public UIOffset value;

    }

    internal unsafe struct PropertySolverInfo_UIOffset {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UIOffset* activeTransitionList;
        ...
        public DataList<PropertySolver_UIOffset.AnimationValue_UIOffset> animationValues;

    }

    internal static unsafe class PropertySolver_UIOffset {

        public static void Invoke(ref PropertySolverInfo_UIOffset solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UIOffset> styleDatabase, UIOffset* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UIOffset value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UIOffset> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UIOffsetWrite> writeBuffer = tempAllocator->AllocateList<UIOffsetWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIOffset transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UIOffset> transitionPrevValues = tempAllocator->AllocateList<UIOffset>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIOffset next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UIOffset prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUIOffset(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIOffset result = UIOffset_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UIOffset active = new ActiveTransition_UIOffset {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UIOffset prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UIOffset.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUIOffset(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UIOffset {

            public ElementId elementId;
            public UIOffset currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UIOffset solverInfo, in SolverParameters parameters, UIOffset defaultValue, UIOffset* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UIOffset* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UIOffset* outputTargets, BumpList<UIOffsetWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UIOffsetWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UIOffset solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIOffset transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UIOffset> SetupFrameData(ref PropertySolverInfo_UIOffset solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UIOffset> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UIOffset>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UIOffset* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UIOffsetWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UIOffset value = update.Get<UIOffset>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UIOffsetWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UIOffset solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UIOffsetWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UIOffset.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UIOffsetWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UIOffset.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UIOffset FindInWriteBuffer(ElementId elementId, BumpList<UIOffsetWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UIOffset FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UIOffset> list) {
        ...
        private static UIOffset ResolveAnimationVariable(ushort variableNameId, UIOffset defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUIOffset(out UIOffset resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<UIOffsetWrite> writeBuffer, ElementMap changeMap, UIOffset* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UIOffset> styleDatabase,
            UIOffset* outputTargets,
            ...
            ref BumpList<UIOffsetWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UIOffset variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UIOffset* values = tempAllocator->Allocate<UIOffset>(usedUpdateSize);
            UIOffset* dbValues = tempAllocator->Allocate<UIOffset>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UIOffset databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UIOffset* current = outputTargets + update.elementId.index;
