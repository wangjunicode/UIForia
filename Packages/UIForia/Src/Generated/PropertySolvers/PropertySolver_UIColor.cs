...
namespace UIForia.Style {
    ...
    internal unsafe struct PropertySolverInfo_UIColor {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_UIColor* activeTransitionList;
        ...
        public DataList<PropertySolver_UIColor.AnimationValue_UIColor> animationValues;

    }

    internal static unsafe class PropertySolver_UIColor {

        public static void Invoke(ref PropertySolverInfo_UIColor solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<UIColor> styleDatabase, UIColor* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_UIColor value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_UIColor> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<UIColorWrite> writeBuffer = tempAllocator->AllocateList<UIColorWrite>(maxWriteBufferSize);
            CheckedArray<ColorVariable> variables = SolverParameters.GetVariableList_ColorVariable(parameters);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIColor transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<UIColor> transitionPrevValues = tempAllocator->AllocateList<UIColor>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIColor next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        UIColor prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromUIColor(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        UIColor result = UIColor_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_UIColor active = new ActiveTransition_UIColor {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_UIColor prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_UIColor.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromUIColor(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_UIColor {

            public ElementId elementId;
            public UIColor currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_UIColor solverInfo, in SolverParameters parameters, UIColor defaultValue, UIColor* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                UIColor* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, UIColor* outputTargets, BumpList<UIColorWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref UIColorWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_UIColor solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_UIColor transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_UIColor> SetupFrameData(ref PropertySolverInfo_UIColor solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_UIColor> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_UIColor>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, UIColor* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<UIColorWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                UIColor value = update.Get<UIColor>();
                ...
                writeBuffer.array[writeBuffer.size++] = new UIColorWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_UIColor solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<UIColorWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_UIColor.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new UIColorWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_UIColor.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static UIColor FindInWriteBuffer(ElementId elementId, BumpList<UIColorWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_UIColor FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_UIColor> list) {
        ...
        private static UIColor ResolveAnimationVariable(ushort variableNameId, UIColor defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ColorVariable> variables, ElementId elementId) {
            ...
            for (int i = 0; i < variables.size; i++) {
                ColorVariable variable = variables[i];
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToUIColor(out UIColor resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ColorVariable> variables, ref BumpList<UIColorWrite> writeBuffer, ElementMap changeMap, UIColor* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<UIColor> styleDatabase,
            UIColor* outputTargets,
            ...
            ref BumpList<UIColorWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                UIColor variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            UIColor* values = tempAllocator->Allocate<UIColor>(usedUpdateSize);
            UIColor* dbValues = tempAllocator->Allocate<UIColor>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                UIColor databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                UIColor* current = outputTargets + update.elementId.index;
