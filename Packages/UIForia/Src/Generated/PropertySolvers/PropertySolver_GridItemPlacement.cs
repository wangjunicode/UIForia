...
namespace UIForia.Style {
    ...
    internal struct GridItemPlacementWrite {

        public ElementId elementId;
        public GridItemPlacement value;

    }

    internal unsafe struct PropertySolverInfo_GridItemPlacement {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_GridItemPlacement* activeTransitionList;
        ...
        public DataList<PropertySolver_GridItemPlacement.AnimationValue_GridItemPlacement> animationValues;

    }

    internal static unsafe class PropertySolver_GridItemPlacement {

        public static void Invoke(ref PropertySolverInfo_GridItemPlacement solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<GridItemPlacement> styleDatabase, GridItemPlacement* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_GridItemPlacement value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_GridItemPlacement> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<GridItemPlacementWrite> writeBuffer = tempAllocator->AllocateList<GridItemPlacementWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_GridItemPlacement transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<GridItemPlacement> transitionPrevValues = tempAllocator->AllocateList<GridItemPlacement>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        GridItemPlacement next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        GridItemPlacement prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromGridItemPlacement(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        GridItemPlacement result = GridItemPlacement_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_GridItemPlacement active = new ActiveTransition_GridItemPlacement {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_GridItemPlacement prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_GridItemPlacement.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromGridItemPlacement(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_GridItemPlacement {

            public ElementId elementId;
            public GridItemPlacement currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_GridItemPlacement solverInfo, in SolverParameters parameters, GridItemPlacement defaultValue, GridItemPlacement* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                GridItemPlacement* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, GridItemPlacement* outputTargets, BumpList<GridItemPlacementWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref GridItemPlacementWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_GridItemPlacement solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_GridItemPlacement transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_GridItemPlacement> SetupFrameData(ref PropertySolverInfo_GridItemPlacement solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_GridItemPlacement> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_GridItemPlacement>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, GridItemPlacement* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<GridItemPlacementWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                GridItemPlacement value = update.Get<GridItemPlacement>();
                ...
                writeBuffer.array[writeBuffer.size++] = new GridItemPlacementWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_GridItemPlacement solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<GridItemPlacementWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_GridItemPlacement.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new GridItemPlacementWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_GridItemPlacement.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static GridItemPlacement FindInWriteBuffer(ElementId elementId, BumpList<GridItemPlacementWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_GridItemPlacement FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_GridItemPlacement> list) {
        ...
        private static GridItemPlacement ResolveAnimationVariable(ushort variableNameId, GridItemPlacement defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            GridItemPlacement value = default;
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToGridItemPlacement(out GridItemPlacement resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<GridItemPlacementWrite> writeBuffer, ElementMap changeMap, GridItemPlacement* outputTargets) {
            ...
            for (int updateIdx = 0; updateIdx < propertyList.size; updateIdx++) {
                ...
                if (start == -1) {
                    ...
                    writeBuffer.array[writeBuffer.size++] = new GridItemPlacementWrite() {
                        elementId = update.elementId,
                        value = update.Get<GridItemPlacement>(),
                    };
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<GridItemPlacement> styleDatabase,
            GridItemPlacement* outputTargets,
            ...
            ref BumpList<GridItemPlacementWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                GridItemPlacement variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            GridItemPlacement* values = tempAllocator->Allocate<GridItemPlacement>(usedUpdateSize);
            GridItemPlacement* dbValues = tempAllocator->Allocate<GridItemPlacement>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                GridItemPlacement databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                GridItemPlacement* current = outputTargets + update.elementId.index;
