...
namespace UIForia.Style {
    ...
    internal unsafe struct PropertySolverInfo_Byte {
        ...
        public DataList<PropertySolver_Byte.AnimationValue_Byte> animationValues;

    }

    internal static unsafe class PropertySolver_Byte {

        public static void Invoke(ref PropertySolverInfo_Byte solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<Byte> styleDatabase, Byte* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_Byte value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_Byte> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<ByteWrite> writeBuffer = tempAllocator->AllocateList<ByteWrite>(maxWriteBufferSize);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_Byte transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<Byte> transitionPrevValues = tempAllocator->AllocateList<Byte>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        Byte next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        Byte prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromByte(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        Byte result = Byte_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_Byte active = new ActiveTransition_Byte {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_Byte prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_Byte.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromByte(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_Byte {
        ...
        private static void WriteDefaultValues(in PropertySolverInfo_Byte solverInfo, in SolverParameters parameters, Byte defaultValue, Byte* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                Byte* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, Byte* outputTargets, BumpList<ByteWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref ByteWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_Byte solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_Byte transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_Byte> SetupFrameData(ref PropertySolverInfo_Byte solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_Byte> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_Byte>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, Byte* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<ByteWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new ByteWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_Byte solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<ByteWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_Byte.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new ByteWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_Byte.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static Byte FindInWriteBuffer(ElementId elementId, BumpList<ByteWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_Byte FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_Byte> list) {
        ...
        private static Byte ResolveAnimationVariable(ushort variableNameId, Byte defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<ValueVariable> variables, ElementId elementId) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToByte(out Byte resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<ValueVariable> variables, ref BumpList<ByteWrite> writeBuffer, ElementMap changeMap, Byte* outputTargets) {
        ...
        private static void HandlePropertyUpdates(
            ...
            ref BumpList<ByteWrite> writeBuffer) {
            ...
            DataList<StylePalette> palettes = default;
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                if (usedUpdates[i].dbValueLocation < 0) {

                    ushort paletteIndex = usedUpdates[i].paletteIndex;
                    ushort propertyIndex = usedUpdates[i].palettePropertyIndex;
                    
                    if (palettes[paletteIndex].TryConvertToByte(propertyIndex, out byte retn)) {
                            
                    }
                    
                }

                Byte variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            Byte* values = tempAllocator->Allocate<Byte>(usedUpdateSize);
            Byte* dbValues = tempAllocator->Allocate<Byte>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                Byte databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                Byte* current = outputTargets + update.elementId.index;
