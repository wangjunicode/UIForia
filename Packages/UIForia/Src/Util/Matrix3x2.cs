...
namespace UIForia.Util {
    ...
    public struct Matrix3x2 {
        ...
        public Matrix3x2 Multiply(in Matrix3x2 secondMatrix) {
        ...
        public static Matrix3x2 Translation(Vector2 translation) {
            return new Matrix3x2(1, 0, 0, 1, translation.x, translation.y);
        }

        public Matrix3x2 Scale(float scaleFactor) {
            return new Matrix3x2(m0 * scaleFactor, m1 * scaleFactor, m2 * scaleFactor, m3 * scaleFactor, m4, m5);
        }

        public Matrix3x2 Scale(float scaleFactorX, float scaleFactorY) {
            return new Matrix3x2(m0 * scaleFactorX, m1 * scaleFactorX, m2 * scaleFactorY, m3 * scaleFactorY, m4, m5);
        }

        public Matrix3x2 Scale(Vector2 scaleFactor) {
            return new Matrix3x2(m0 * scaleFactor.x, m1 * scaleFactor.x, m2 * scaleFactor.y, m3 * scaleFactor.y, m4, m5);
        }

        public Matrix3x2 Rotate(float angle) {
            float ca = Mathf.Cos(angle * Mathf.Deg2Rad);
            float sa = Mathf.Sin(angle * Mathf.Deg2Rad);

            return new Matrix3x2((m0 * ca + m2 * sa), (m1 * ca + m3 * sa), (m2 * ca - m0 * sa), (m3 * ca - m1 * sa), m4, m5);
        }

        public Matrix3x2 Translate(float x, float y) {
            return new Matrix3x2(m0, m1, m2, m3, m0 * x + m2 * y + m4, m1 * x + m3 * y + m5);
        }

        public Matrix3x2 Translate(Vector2 position) {
            return new Matrix3x2(m0, m1, m2, m3, m0 * position.x + m2 * position.y + m4, m1 * position.x + m3 * position.y + m5);
        }

        public Matrix3x2 SkewX(float angle) {
            float ta = Mathf.Tan(angle * Mathf.Deg2Rad);
            return new Matrix3x2(m0, m1, (m2 + m0 * ta), (m3 + m1 * ta), m4, m5);
        }

        public Matrix3x2 SkewY(float angle) {
            float ta = Mathf.Tan(angle * Mathf.Deg2Rad);
            return new Matrix3x2((m0 + m2 * ta), (m1 + m3 * ta), m2, m3, m4, m5);
        }
        ...
        public static Matrix3x2 TRS(Vector2 position, float rotation, Vector2 scale) {
            const float a = 1;
            const float b = 0;
            const float c = 0;
            const float d = 1;
        ...
        public static Matrix3x2 TRS(float positionX, float positionY, float rotation, float scaleX, float scaleY) {
        ...
        public static Matrix3x2 TranslateScale(float positionX, float positionY, float scaleX, float scaleY) {
            return new Matrix3x2(
                scaleX,
                0,
                0,
