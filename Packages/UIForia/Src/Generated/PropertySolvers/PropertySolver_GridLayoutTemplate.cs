...
namespace UIForia.Style {
    ...
    internal struct GridLayoutTemplateWrite {
    ...
    internal unsafe struct PropertySolverInfo_GridLayoutTemplate {

        public int enumTypeId;
        public bool isImplicitInherit;

        public PendingTransition* pendingTransitionList;
        public ActiveTransition_GridLayoutTemplate* activeTransitionList;
        ...
        public DataList<PropertySolver_GridLayoutTemplate.AnimationValue_GridLayoutTemplate> animationValues;

    }

    internal static unsafe class PropertySolver_GridLayoutTemplate {

        public static void Invoke(ref PropertySolverInfo_GridLayoutTemplate solverInfo, in SolverParameters parameters, ref PropertySolverContext context, DataList<GridLayoutTemplate> styleDatabase, GridLayoutTemplate* outputTargets, int solverIndex, PropertyId propertyId, BumpAllocator* tempAllocator, bool contextual) {
            ...
            if (contextual) {
                ...
                for (int i = 0; i < solverInfo.animationValues.size; i++) {
                    ref AnimationValue_GridLayoutTemplate value = ref solverInfo.animationValues[i];
                    outputTargets[value.elementId.index] = value.currentValue;
                }

            }

            BumpList<ActiveTransition_GridLayoutTemplate> purgatoryTransitionList = SetupFrameData(
                ref solverInfo,
                parameters,
                tempAllocator,
                scratchMap,
            ...
            BumpList<GridLayoutTemplateWrite> writeBuffer = tempAllocator->AllocateList<GridLayoutTemplateWrite>(maxWriteBufferSize);
            CheckedArray<GridTemplateVariable> variables = SolverParameters.GetVariableList_GridTemplateVariable(parameters);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_GridLayoutTemplate transition = ref solverInfo.activeTransitionList[i];
            ...
            BumpList<GridLayoutTemplate> transitionPrevValues = tempAllocator->AllocateList<GridLayoutTemplate>(changedTransitionIds.size);
            ...
            if (keyFramesProperties.size > 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        GridLayoutTemplate next = keyFrameProp.next == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.next];
                        GridLayoutTemplate prev = keyFrameProp.prev == KeyFrameResult.k_Current ? outputTargets[keyFrameProp.elementId.index] : styleDatabase[keyFrameProp.prev];
                        ...
                        keyFramesResultValues[i] = InterpolatedStyleValue.FromGridLayoutTemplate(next, prev, keyFrameProp.t);
                    ...
                    for (int i = 0; i < keyFramesProperties.size; i++) {
                        ...
                        GridLayoutTemplate result = GridLayoutTemplate_Interpolator.Interpolate(prev, next, keyFrameProp.t);
            ...
            if (changedTransitionIds.size != 0) {
                ...
                if (contextual) {
                    ...
                    for (int i = 0; i < transitionPrevValues.size; i++) {
                        ...
                        ActiveTransition_GridLayoutTemplate active = new ActiveTransition_GridLayoutTemplate {
                        ...
                        if (scratchMap.Get(elementId)) {
                            ActiveTransition_GridLayoutTemplate prevTransition = FindInPurgatory(elementId, purgatoryTransitionList);
                            float progress = math.clamp(prevTransition.elapsed / (float) prevTransition.definition.duration, 0, 1);
                            active.prev = ActiveTransition_GridLayoutTemplate.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, prevTransition, progress);
                        }
                        ...
                        keyFramesResultValues.Add(InterpolatedStyleValue.FromGridLayoutTemplate(outputTargets[elementId.index], transitionPrevValues.array[i], 0));
        ...
        public struct AnimationValue_GridLayoutTemplate {

            public ElementId elementId;
            public GridLayoutTemplate currentValue;

        }

        private static void WriteDefaultValues(in PropertySolverInfo_GridLayoutTemplate solverInfo, in SolverParameters parameters, GridLayoutTemplate defaultValue, GridLayoutTemplate* outputTargets, BumpAllocator* tempAllocator, ElementMap previousDefinitionMap, ElementMap definitionMap, ElementMap changeMap) {
            ...
            for (int i = 0; i < defaults.size; i++) {
                ...
                GridLayoutTemplate* dest = outputTargets + defaults.array[i].index;
        ...
        private static void FlushWriteBuffer(in SolverParameters parameters, GridLayoutTemplate* outputTargets, BumpList<GridLayoutTemplateWrite> writeBuffer) {

            // maybe sort the write buffer by elementId for better write locality?

            for (int i = 0; i < writeBuffer.size; i++) {
                ref GridLayoutTemplateWrite entry = ref writeBuffer.array[i];
        ...
        private static void RemoveExpiredTransitions(ref PropertySolverInfo_GridLayoutTemplate solverInfo, in PropertySolverContext context) {
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ref ActiveTransition_GridLayoutTemplate transition = ref solverInfo.activeTransitionList[i];
        ...
        private static BumpList<ActiveTransition_GridLayoutTemplate> SetupFrameData(ref PropertySolverInfo_GridLayoutTemplate solverInfo, in SolverParameters parameters, BumpAllocator* tempAllocator, ElementMap scratch, ElementMap inheritMap, ElementMap previousDefinitionMap) {
            ...
            BumpList<ActiveTransition_GridLayoutTemplate> purgatoryTransitionList = tempAllocator->AllocateList<ActiveTransition_GridLayoutTemplate>(solverInfo.activeTransitionCount);
        ...
        private static void UpdateInstanceProperties(ref PropertySolverContext context, GridLayoutTemplate* outputTargets, InstancePropertyUpdateList instanceUpdateList, ElementMap definitionMap, ElementMap activeTransitionMap, ElementMap changeMap, ref BumpList<GridLayoutTemplateWrite> writeBuffer) {
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                GridLayoutTemplate value = update.Get<GridLayoutTemplate>();
                ...
                writeBuffer.array[writeBuffer.size++] = new GridLayoutTemplateWrite() {
        ...
        private static void UpdateActiveTransitions(in PropertySolverInfo_GridLayoutTemplate solverInfo, in SolverParameters parameters, ElementMap animationMap, ElementMap resetTransitionMap, ref BumpList<GridLayoutTemplateWrite> writeBuffer) {
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                if (resetTransitionMap.Get(transition.elementId)) {
                    ...
                    transition.prev = ActiveTransition_GridLayoutTemplate.ResolveTransitionResetValue(solverInfo.enumTypeId, parameters, transition, progress);
            ...
            for (int i = 0; i < solverInfo.activeTransitionCount; i++) {
                ...
                writeBuffer.array[writeBuffer.size++] = new GridLayoutTemplateWrite() {
                    elementId = transition.elementId,
                    value = ActiveTransition_GridLayoutTemplate.Interpolate(solverInfo.enumTypeId, parameters, transition, progress)
                };
        ...
        private static GridLayoutTemplate FindInWriteBuffer(ElementId elementId, BumpList<GridLayoutTemplateWrite> list) {
            int idx = 0;
        ...
        // value is 100% in the buffer 
        private static ActiveTransition_GridLayoutTemplate FindInPurgatory(ElementId elementId, BumpList<ActiveTransition_GridLayoutTemplate> list) {
        ...
        private static GridLayoutTemplate ResolveAnimationVariable(ushort variableNameId, GridLayoutTemplate defaultValue, CheckedArray<TraversalInfo> traversalInfo, CheckedArray<GridTemplateVariable> variables, ElementId elementId) {
            ...
            for (int i = 0; i < variables.size; i++) {
                GridTemplateVariable variable = variables[i];
            ...
            GridLayoutTemplate value = default;
            ...
            for (int i = start; i < end; i++) {
                ...
                if (depthDiff > nearestDistance || !variable.TryConvertToGridLayoutTemplate(out GridLayoutTemplate resolved)) {
                    continue;
                }
        ...
        private static void ResolveVariables(int enumTypeId, CheckedArray<TraversalInfo> traversalInfo, ref DataList<PropertyContainer> propertyList, CheckedArray<GridTemplateVariable> variables, ref BumpList<GridLayoutTemplateWrite> writeBuffer, ElementMap changeMap, GridLayoutTemplate* outputTargets) {
            ...
            for (int updateIdx = 0; updateIdx < propertyList.size; updateIdx++) {
                ...
                if (start == -1) {
                    ...
                    writeBuffer.array[writeBuffer.size++] = new GridLayoutTemplateWrite() {
                        elementId = update.elementId,
                        value = update.Get<GridLayoutTemplate>(),
                    };
        ...
        private static void HandlePropertyUpdates(
            ...
            int updateCount,
            DataList<GridLayoutTemplate> styleDatabase,
            GridLayoutTemplate* outputTargets,
            ...
            ref BumpList<GridLayoutTemplateWrite> writeBuffer) {
            ...
            for (int i = 0; i < usedUpdateSize; i++) {
                ...
                GridLayoutTemplate variableDefaultValue = styleDatabase[usedUpdates[i].dbValueLocation];
            ...
            GridLayoutTemplate* values = tempAllocator->Allocate<GridLayoutTemplate>(usedUpdateSize);
            GridLayoutTemplate* dbValues = tempAllocator->Allocate<GridLayoutTemplate>(usedUpdateSize);
            ...
            for (int i = 0; i < updateCount; i++) {
                ...
                GridLayoutTemplate databaseValue = styleDatabase[update.dbValueLocation];

                inheritMap.Unset(update.elementId); // todo -- how do I handle inherit? db value? or some other place?

                GridLayoutTemplate* current = outputTargets + update.elementId.index;
