...
namespace UIForia.Style {
    ...
    internal unsafe struct CustomPropertySolver_float2 : ICustomPropertySolver {

        public DataList<PropertyUpdate> sharedProperties;
        public DataList<Transition<float2>> transitions;
        public UnsafeHashMap<ElementId, float2> valueMap;

        public float2 defaultValue;

        public float2 GetValue(ElementId elementId, float2 defaultOverride, bool useDefault) {

            if (!valueMap.TryGetValue(elementId, out float2 retn)) {
        ...
        public void Solve(PropertyId propertyId, ref SolverParameters parameters, ref PropertyTables propertyTables, ref CustomSolverInfo sharedSolverInfo, BumpAllocator* tempAllocator) {
            ...
            DataList<float2> dbValues = propertyTables.propertyTable_float2;
            ...
            CheckedArray<VectorVariable> variableList = sharedSolverInfo.GetVariableList_VectorVariable();
            ...
            if (transitionUpdateList.size > 0) {
                ...
                for (int i = 0; i < transitionUpdateList.size; i++) {
                    ...
                    for (int j = 0; j < transitions.size; j++) {
                        ref Transition<float2> transition = ref transitions[j];
                    ...
                    if (!found) {
                        ...
                        if (!valueMap.TryGetValue(elementId, out float2 prevValue)) {
                            prevValue = defaultValue;
                        }

                        TransitionDefinition transitionDefinition = parameters.transitionDatabase[transitionId];

                        transitions.Add(new Transition<float2>() {
            ...
            for (int i = 0; i < instanceUpdateList.size; i++) {
                ...
                float2 value = update.Get<float2>();
            ...
            for (int i = 0; i < sharedProperties.size; i++) {
                ...
                float2 dbValue = dbValues[update.dbValueLocation];
        ...
        private void ApplyAnimations(ref SolverParameters parameters, CheckedArray<VectorVariable> variableList, DataList<KeyFrameResult> keyFramesProperties, DataList<float2> dbValues, int defaultIndex, ElementMap animationMap) {
            ...
            for (int i = 0; i < keyFramesProperties.size; i++) {
                ...
                if (!valueMap.TryGetValue(keyFrameProp.elementId, out float2 currentValue)) {
                ...
                float2 next = keyFrameProp.next == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.next];
                float2 prev = keyFrameProp.prev == KeyFrameResult.k_Current ? currentValue : dbValues[keyFrameProp.prev];
                ...
                float2 result = float2_Interpolator.Interpolate(prev, next, keyFrameProp.t);
        ...
        private void ApplyTransitions(ref SolverParameters parameters, ElementMap animationMap) {
            for (int i = 0; i < transitions.size; i++) {
                ...
                ref Transition<float2> transition = ref transitions[i];

                if (parameters.initMap.Get(transition.elementId) || animationMap.Get(transition.elementId)) {
                    continue;
                }

                if (!valueMap.TryGetValue(transition.elementId, out float2 currentValue)) {
                ...
                switch (transition.state) {
                    ...
                    case TransitionState.Active: {
                        ...
                        float progress = 0;
                        float2 output = transition.prevValue;
                        ...
                        if (transition.elapsedTime >= transition.delay) {
                            ...
                            output = float2_Interpolator.Interpolate(transition.prevValue, transition.nextValue, t);
                        }
        ...
        private float2 ResolveAnimationVariable(ushort nextVarId, float2 current, CheckedArray<TraversalInfo> traversalInfoTable, CheckedArray<VectorVariable> variableList, ElementId elementId) {
            ...
            int targetIndex = -1;
            float2 resolved = default;

            for (int v = 0; v < variableList.size; v++) {

                ref VectorVariable variable = ref variableList.Get(v);
                ...
                if (depthDiff > nearestDistance || !traversalInfo.IsSelfOrDescendentOf(variable.traversalInfo) || !variable.TryConvertTofloat2(out resolved)) {
                    continue;
                }
        ...
        private void ResolveVariables(DataList<PropertyContainer> variableResolveList, CheckedArray<VectorVariable> variableList, CheckedArray<TraversalInfo> traversalInfoTable) {
        ...
        public void Initialize() {
            valueMap = new UnsafeHashMap<ElementId, float2>(31, Allocator.Persistent);
            sharedProperties = new DataList<PropertyUpdate>(16, Allocator.Persistent);
            transitions = new DataList<Transition<float2>>(8, Allocator.Persistent);
        }
